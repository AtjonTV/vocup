...
namespace Vocup.Forms
{
    public partial class PracticeCountDialog : Form
    {
        ...
        {
            var compositor = new ListCompositor<VocabularyWord>();

            var unpracticedItems = book.Words
                .Where(x => x.PracticeState == PracticeState.Unpracticed);
            var wronglyPracticedItems = book.Words
                .Where(x => x.PracticeState == PracticeState.WronglyPracticed);
            var correctlyPracticedItems = book.Words
            ...
            if (RbAllStates.Checked)
            {
                compositor.AddSource(unpracticedItems.ToList(),
                    count * Settings.Default.PracticePercentageUnpracticed / 100d);

                compositor.AddSource(wronglyPracticedItems.ToList(),
                    count * Settings.Default.PracticePercentageWrong / 100d);

                compositor.AddSource(correctlyPracticedItems.ToList(),
                    count * Settings.Default.PracticePercentageCorrect / 100d);
            }
            ...
            var resultList = compositor.ToList(count);

            foreach (var item in resultList) PracticeList.Add(new VocabularyWordPractice(item));
